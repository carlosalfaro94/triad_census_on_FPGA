...
/**
  ...
  *   header file graph.h. Please refer to it to check the documentation.
  *
...
void reorder_and_retag(GRAPH* g, int (*function) (const void*, const void*)){
    ...
    if (NULL == ids){
        printf("Error allocating memory for ids\n");
        return;
    }
    ...
    for(i=0;i<num_nodes;i++){
        ...
        qsort(get_adj_list(n), get_num_neighbors(n), sizeof(EDGE), &comp_edges_by_id);
        set_node_id(n,i);
    }
