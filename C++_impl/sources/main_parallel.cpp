...
int main(int argc, char **argv){
    ...
    #if 0
    if (is_BM_kernel(name_kernel) && !is_NDRange_kernel(name_kernel)){
	cl_mem local_node_list = clCreateBuffer(context, CL_MEM_READ_ONLY, num_nodes*sizeof(NODE_DEVICE), NULL, &err);
	check_error(err, "Error creating local node list buffer");
	cl_mem local_edge_list = clCreateBuffer(context, CL_MEM_READ_ONLY, num_edges*sizeof(EDGE_DEVICE), NULL, &err);
        check_error(err, "Error creating local edge list buffer");
        err = clSetKernelArg(kernel, 4, sizeof(cl_mem), (void*) &local_node_list);
        check_error(err, "Error setting kernel arg 4");
        err = clSetKernelArg(kernel, 5, sizeof(cl_mem), (void*) &local_edge_list);
        check_error(err, "Error setting kernel arg 5");
        err = clSetKernelArg(kernel, 6, sizeof(cl_uint), (void *) &num_edges);
        check_error(err, "Error setting kernel arg 6");
    }
    #endif
